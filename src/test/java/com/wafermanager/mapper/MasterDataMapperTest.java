...
class MasterDataMapperTest extends AbstractTest {
    ...
    void create() {
        for (int i = 0, l = masterDataArr.length; i < l; i++) {
            int _i = i;
            assertThrows(Exception.class, () -> masters.create(masterDataArr[_i]));
        }
        MasterData md = new MasterData("-1", -1, -1, -1, "-1", LocalDateTime.now());
        assertEquals(1, masters.create(md));
    }

    @Test
    void read() {
        for (int i = 0, l = masterDataArr.length; i < l; i++) {
            assertEquals(masterDataArr[i], masters.read(masterDataArr[i].getUid()));
        }
    }
    ...
    void list() {
        ...
        retrieved = masters.list(null,3, 0);
        assertEquals(3, retrieved.size());
        retrieved.forEach(e -> assertTrue(_masterDataList.contains(e)));

        retrieved = masters.list(null,3, 1);
        assertEquals(3, retrieved.size());
        retrieved.forEach(e -> assertTrue(_masterDataList.contains(e)));

        retrieved = masters.list(null,3, 2);
        assertEquals(3, retrieved.size());
        retrieved.forEach(e -> assertTrue(_masterDataList.contains(e)));

        retrieved = masters.list(null,3, 3);
        assertEquals(1, retrieved.size());
        retrieved.forEach(e -> assertTrue(_masterDataList.contains(e)));
    }
